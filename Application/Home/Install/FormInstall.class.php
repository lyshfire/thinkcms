<?php
	...
	class FormInstall {	
		...
		public function getAgreement(){
			...
			$license = dirname(__FILE__).'/license.txt';
			$fp=fopen($license, "r");
			if (!$fp){
				return 'Failed to open file :'.$license;
			}
			while(!feof($fp)){
				$fc.='<li>'.fgets($fp).'</li>';
			}
		...
		function check(){
			...
			$fc.='<span><img src="./Application/Home/Install/images/correct.gif" /></span>';
			$fc.='</li>';
			...
			if(phpversion() >= '5.2.0'){
				...
				$fc.='<span class="red-font"><img src="./Application/Home/Install/images/error.gif" />&nbsp;无法安装</span>';
				$ok=false;
			}
			$fc.='</li>';
			...
			if(extension_loaded('json')){
				...
				$fc.='<span class="red-font"><img src="./Application/Home/Install/images/error.gif" />&nbsp;不只持json,<a href="http://pecl.php.net/package/json" target="_blank">安装 PECL扩展</a></span>';
				$ok=false;
			}
			$fc.='</li>';
			...
			if(extension_loaded('gd') && $stas){
				...
				$fc.='<span class="red-font"><img src="./Application/Home/Install/images/error.gif" />&nbsp;不支持缩略图和水印</span>';
				$ok=false;
			}
			$fc.='</li>';
			...
			if(ini_get('allow_url_fopen')){
				...
				$fc.='<span class="red-font"><img src="./Application/Home/Install/images/error.gif" />&nbsp;不支持保存远程图片</span>';
				$ok=false;
			}
			$fc.='</li>';
...
//				$fc.='<img src="./Application/Home/Install/images/correct.gif" />&nbsp;可写';	
//			}else{
//				$fc.='<img src="./Application/Home/Install/images/error.gif" />&nbsp;不可写';
//			}
//		
//			$fc.='</span>';
//			$fc.='<span>文件必须可写</span>';
...
//			$fc.='</li>';
			...
			$fc.='<span>';
			if( is_writable("./Application/Runtime")){ 
				$fc.='<img src="./Application/Home/Install/images/correct.gif" />&nbsp;可写';	
			}else{
				$fc.='<img src="./Application/Home/Install/images/error.gif" />&nbsp;不可写';
			}
